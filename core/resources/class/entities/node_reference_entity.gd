...
## [code]super.set_reference()[/code], or manually emit [signal reference_set] 
## in case your widget needs to be notified about changes to the reference.
func set_reference(node: ClassNode) -> void:
	node_reference = node
	reference_set.emit()

## Returns wether the given node is accepted by this entity.[br][br]
##
